    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="border-b border-[#dbe7f6] bg-[#f8fbff] px-6 py-5">
        <div className="flex flex-wrap items-end gap-2">
		  ...
		  {canUpload ? <input ref={inputRef} type="file" className="hidden" accept=".pdf,.docx,.md,.txt,.html,.csv" onChange={(event) => void upload(event.target.files?.[0])} /> : null}
		  {canUpload ? <Button type="button" onClick={() => inputRef.current?.click()} disabled={uploading}>{uploading ? <Loader2Icon className="size-4 animate-spin" /> : <UploadIcon className="size-4" />}上传文件</Button> : null}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto divide-y divide-[#e3edf9]">
        {collections.length === 0 && !loading ? <div className="p-8 text-center text-sm text-muted-foreground">暂无 FastGPT 文件。</div> : collections.map((item) => (
          <div key={item.id} className="flex items-center gap-3 px-6 py-4">
			...
			{canDelete ? <Button type="button" variant="ghost" size="icon" className="text-destructive" onClick={() => void remove(item.id)} disabled={deletingId === item.id} aria-label="删除文件">{deletingId === item.id ? <Loader2Icon className="size-4 animate-spin" /> : <Trash2Icon className="size-4" />}</Button> : null}
          </div>
        ))}
      </div>
    </div>
