...
const formSchema = z.object({
  storeId: z.string().trim().refine((value) => Number(value) > 0, "请选择门店账号"),
...
export function FastGPTProvisionDialog({ open, onOpenChange, onProvisioned }: FastGPTProvisionDialogProps) {
  ...
  useEffect(() => {
    ...
    void fetchWxWorkProtocolInstances({ limit: 200 })
      ...
      .catch((error) => {
        if (!cancelled) {
          toast.error(error instanceof Error ? error.message : "加载门店账号失败")
        }
      })
      .finally(() => {
        if (!cancelled) {
    ...
    >
      <form id="fastgpt-provision-form" className="space-y-4" onSubmit={handleSubmit(submit)}>
        <Field data-invalid={Boolean(errors.storeId)}>
          <FieldLabel htmlFor="fastgpt-store">门店账号</FieldLabel>
          <FieldContent>
            <Controller
              control={control}
              name="storeId"
              render={({ field }) => (
                <OptionCombobox
                  ...
                  placeholder={loadingInstances ? "正在加载门店账号" : "选择要创建知识库的门店账号"}
                  searchPlaceholder="搜索门店账号"
                  emptyText="暂无可用门店账号"
                />
              )}
            />
