      ...
      onMessage: (event, socket) => {
        try {
          ...
          if (eventType === "notification.created") {
            ...
            if (
              !notification ||
              notification.notificationType !== "manual_handoff_created" ||
              notification.bizType !== "conversation" ||
              !notification.bizId
            ) {
              return
            }
            toast(notification.title || t("conversation.manualHandoffToastTitle"), {
              description: notification.content,
              action: {
                label: t("conversation.openConversation"),
                onClick: () => {
                  store.setConversationFilter("pending")
                  void store.loadConversations().then(() =>
                    store.selectConversation(notification.bizId ?? 0)
                  )
                },
              },
            })
          ...
          if (eventType === "customer_tag.changed" && conversationId > 0) {
            void store.refreshConversationDetail(conversationId).catch((error) => {
              toast.error(error instanceof Error ? error.message : "刷新客户长期标签失败")
            })
            return
          }
