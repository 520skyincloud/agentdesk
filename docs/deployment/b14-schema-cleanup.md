...
> 当前状态：工具已具备，生产执行仍为 `No-Go`。部署方已声明 FastGPT Base URL 与 Integration Token 可用，但仍必须在 B13 目标主机验证 HTTPS、同环境配对和真实调用，并完成目标 MySQL、丽斯文旅 / 高铁南站店全链验收、正式停机、仓库外加密备份和独立恢复验证。
