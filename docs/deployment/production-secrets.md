...
2026-07-24，业务方确认把当前执行主机无法完成的 `production.env` 独立验证延期到正式部署前。该决定只改变验证时间，不降低或取消任何发布门禁：

- `production.env` 不属于源码或 PR 交付物；当前主机无法访问消息临时附件和目标部署用户的受限文件，不阻止统一分支继续代码评审或合并。
- 文件权限、16 项变量、冻结 SHA-256、Compose 解析、FastGPT HTTPS/鉴权和目标数据库连通性只能在实际部署主机验证；部署方声明不能替代现场证据。
- 延期期间 PR、合并提交和构建产物必须继续标记为 `No-Go`，不得据此切换正式 `8083`、启动生产 worker 或形成 pilot 验收结论。
- B14 物理删除仍必须等待 B13 全部验收、正式停机、仓库外加密备份和独立恢复验证；固定 `7 表、5 列、4 索引` 白名单不得扩大。
