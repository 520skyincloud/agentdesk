...
| **2** | 写 B 类两阶段工具循环：`tool_loop` + `tool_executor`（取数循环 + 最终表达）| 保留 V1 工具 | 外部 MCP 单测 |
| **3** | A 类动作迁移：工单 graph → actions executor（后端决定要不要建单，模型填内容）| 保留旧 graph | 工单单测 |
| **4** | 技能确定性注入（后续做，当前未启用）| 保留旧 middleware | 技能单测 |
| **5** | 删 ADK + 45 行常量 | 保留 2 周期 release | 全量回归 + 真实消息 |

> **顺序调整说明**：按用户决策「先翻默认值再补工具」，阶段 1 先让 V2 成为默认主链（移除白名单限制），
> 工单/搜索/技能在翻默认值后短暂不可用，随后阶段 2~4 补回。核心场景（知识问答、转人工、发定位/小程序/电话）
> 在 V2 已走确定性路径，翻默认值后不受影响。
