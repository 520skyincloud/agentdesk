...
## 84. 当前实施检查点：客服档案与综合客服组父锁闭环（2026-07-15）

第 83 批已把 Team 删除依赖检查放进持锁事务，但客服档案创建、跨组移动和删除仍先在事务外读取档案、权限、账号角色及小组成员关系，再单独写入。多实例下可能在 Team 删除检查后新增档案，或在跨组移动时基于旧档案覆盖并发变化。

- AgentProfileRepository 增加按 TenantID 的 GetForUpdateInTenant。更新和删除先锁当前客服档案，创建直接进入事务；三类写入随后按客服组 ID 升序锁定目标组，跨组更新同时锁原组和目标组。
- 当前租户、综合组存在性/删除状态、管理员或组长管理职责、关联账号、客服角色、账号与客服组同租户、工号唯一性及客服小组成员限制全部在持锁事务内复核。最终更新和删除继续保留 TenantID 写条件。
- 创建持有目标 Team 锁直至档案插入提交。Team 删除先持有父锁时，等待中的创建会看到 Team 已删除并拒绝；创建先持有父锁时，删除随后能看到档案依赖并拒绝。跨组更新按固定 Team ID 顺序，避免两个相反方向移动形成反向父锁序。
- 更新和删除的档案行锁阻止同一档案并发编辑、移动或删除互相覆盖。客服仍属于任一启用小组时，跨综合组移动和删除继续拒绝，且拒绝事务不会改变档案。
- 待派发会话重试只在创建或更新事务成功提交后触发，不在档案或 Team 行锁内执行。API、DTO、页面、权限点和原有操作路径均不变化。
- 测试直接观察 GORM locking clause：创建锁 Team 2；从 Team 2 移动到 Team 1 先锁档案并按 1、2 锁父组；删除锁档案和 Team 2。小组拒绝测试确认移动、删除失败后 TeamID 和展示名不变；跨租户与租户内工号唯一性回归继续通过。
