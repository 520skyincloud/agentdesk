...
## 2026-09-23 客户目标、事实与回复职责收口

当前回复链路的职责固定为：

```text
JEV Intent：识别客户当前业务目标、话语作用和与上一业务目标的关系
PMS / 知识检索：提供结构化事实
Knowledge Judge：只裁决知识候选与知识事实
Generate：根据当前目标选择相关事实，组织自然客服回复
本地代码：校验协议、关键字面值、内部信息泄漏、权限、幂等和发送安全
```

JEV 读取同会话最近唯一业务 Task 的实体、已确认事实和缺失方面，区分新请求、追问、
原因、推荐、候选选择、确认、纠正、不满和取消。客户补充换房原因、要求客服推荐、
选择已给出的房号或纠正手机号时继续当前目标，不重新建立同类流程；手机号和内部订单
定位以结构化 Entity 传递，不再混入客户可见的 `resolvedText`。

PMS 在知识裁决前执行，只把按 Task 归属的 `pms_` 事实和缺失方面写入 ReplyPlan，
不生成 `pms_customer_answer`，也不预写最终客服答案。知识库与 PMS 可以服务同一个 Task，
但 Judge 只裁决知识候选；知识主体增加酒店会员、影视会员、商品、房间、外卖和客房服务
等域隔离，避免把同名但不同主体的知识交给 Generate。

有事实的 Task 不锁定 `AnswerText`。Generate 可以省略与当前问题无关的库存和内部判断，
自然承接客户的纠正、不满、原因和推荐请求，但不得改变事实确定程度或声称只读操作已经
办理。只有没有事实可供重写的服务端固定控制话术继续确定性锁定。单一文本 Task 允许自然
文本；多个文本 Task 才强制 `replyParts`，保持 Task 数量、ID 和顺序，并合并为最多三条消息。
`coveredFactIds` 只声明实际采用的事实，不要求展示全部查询结果；本地只校验声明引用合法及
其电话、地址、数量、金额、日期等关键值，不再做主体、范围、肯否、能力或方法的二次语义裁决。

Generate 协议失败时，知识 Task 可以使用已经选择的客户可见事实安全恢复；PMS Task 不直接
拼接原始事实、库存、冲突统计或内部 ID，只返回具体缺失字段追问，或提示已经查到信息但未能
可靠整理。所有客户文本继续拦截内部 JSON、Task 字段、`pms_` 标记、预订单/接待单 ID 和
`supportedFacts/missingAspects/coveredFactIds` 等内部协议内容。

## 2026-09-23 PMS 只读会话定位与 Judge 有界恢复

PMS 客户定位信息只允许从当前消息、当前会话历史，或最近一次真实成功并已发送的 PMS Task
恢复。RunLog 恢复必须同时满足同 conversation、同 Agent、同 session、PMS 工具状态为
`ok`、对应 Task 含 `pms_` 事实且 CommitMessage 状态为 `sent`；不得从 Intent、未发送 Task
或多条日志拼接定位。当前消息的新手机号、否定和纠正优先。成功唯一定位住宿后，真实预订单
和接待单 ID 只作为内部 Task 定位标记，Generate 不得向客户复述完整手机号或内部订单 ID。

续住、升房、换房、延退仍是只读评估。续住计划同时查询预订单和接待单，并可从旧离店日及
客户明确的续住天数派生目标日期；库存、会员和价格只能使用真实接口字段。可售不等于锁房，
空价格不等于免费，未知目标房型必须追问，任何路径均不得调用 `renew` 或其他 PMS 写操作。

同一预订单可能包含多个接待单。只有当候选记录的预订单 ID、入住离店区间及 PMS 真实房型
标识全部一致时，才允许共享离店日、房型等订单级只读字段，继续查询库存或评估续住；房型
标识优先读取 HPMS 实际返回的 `roomId`，并兼容文档中的 `productId/roomTypeId`。需要具体
接待单 ID 的查询或任何房间级操作仍必须唯一定位，禁止从多房记录中任意选择一个接待单。
只读续住可由预订单的离店日、当前房型及目标日期库存完成可行性回答；接待单和换单续住候选
属于增强信息，不得在预订单与同房型完整库存已确认时把答案降级为资料不足。库存接口即使
返回所有房型，也只校验和展示本次绑定的当前房型；无关房型缺少逐日数据不能误伤当前结果。

Judge 仍是知识语义裁决的唯一责任方。本地只校验协议与发送安全。完整 Judge 首次超时后，
允许在同一总预算内执行一次紧凑恢复；恢复只保留每个知识层得分最高的候选，Task、知识层、
Candidate ID、事实协议与覆盖协议不变。两次失败继续安全失败，不新增关键词答案、业务语义
门槛或第二个 Judge。JEV 合法逆序边界只做原文位置排序；非法或重复边界最多重试一次。

Task 失败继续按 Task 隔离。PMS 已确认的部分事实必须保留并对缺失字段进行局部追问；全局
Generate/Validate 失败只有在唯一未提交文本 Task 时可恢复，不能覆盖已提交或已成功的相邻
Task。只有不携带事实的服务端固定控制话术锁定 `AnswerText`；知识或 PMS 已有事实时，由
Generate 根据客户当前目标组织最终回复，不能用旧预写句覆盖上下文。维修请求在客户明确拒绝
人工时说明真实处理边界，不自动转接、建单或声称处理完成。

客户级 `auto_handoff_enabled=false` 只关闭未经明确授权的自动升级，不能覆盖当前消息明确的
人工诉求、知识库明确“转接”裁决或安全风险路由。普通投诉、知识不足、PMS 失败和服务请求
仍不得借此转人工；授权路由成功后沿用现有幂等服务，只发送一次固定成功话术。

最终 `e5454cd` 在 test-2 的普通客户需求矩阵为 22/22，全部保持 `AI_SERVING`，PMS 只出现
预订单、接待单、会员、库存和房态查询，未创建 `PMSOperation`。明确人工两种表达和门店知识
精确转接两种表达分别在四个独立会话通过，均只发送一次 `帮您转接到同事了` 并进入
`STORE_WECOM_MANUAL`。知识优先级验收必须使用真实胜出层：门店正文覆盖通用转接，只有门店
精确“转接”且无竞争正文时才可把该轮记为知识驱动人工路由。

功能通过不等于性能达标。最终矩阵最慢一轮为 83.178 秒；当前记录只证明回复、路由、只读工具
和投递结果正确，后续性能工作需另设 P90/P95 与分阶段耗时目标。

## 2026-09-16 入住目标与配套资源绑定

基于已恢复的“薇薇”程序 `5cfc219` 修复入住卡片丢失；以下九月九日发布记录
为历史状态，不代表本次恢复后的运行版本。备份本身不修改。
模型识别的 `hotel_info/checkin_process` 与明确的
`service_request/checkin_action + action_request` 均绑定既有入住小程序。
只接受当前来源中的有效目标或已补全回指；取消、已完成、时间、价格及未解歧目标不自动附卡。
V2 使用已验证的主来源，不因客户同轮重发原话而再次猜测唯一来源。

覆盖核对按客户目标计数：与入住正文原话和完整来源相同、且唯一归属的卡片通过
`attachedResources` 标注为配套交付物，不作为第二个客户问题或无知识证据任务。
独立索取小程序仍保留独立目标。局部修复跟随父目标保留卡片身份；父目标不再是
入住办理时移除隐式卡片。仅附属资源不参与客户问题来源排序，完整执行计划仍交给
原 Commit/Outbox，最终 Intent、ActionLedger、ReplyPlan 和资源预期一起更新。
不新增模型调用、外部字段或发送通道；验证及发布边界见开发交接记录。
...
PMS 只读客户目标仍由同一次 Intent 拆成 Task，但具体接口步骤不再由 Generate 自由编排。
Judge 完成知识裁决后，Executor 在 Generate 前按 Task 执行固定只读计划：订单定位、日期区间、
库存/房态、会员权益、目标房型和差价评估。查询成功事实写入该 Task 的 `SupportedFacts`，
未确认方面写入 `MissingAspects`；Generate 只组织这些事实，不再调用已经完成的 `pms_query`。

PMS 与知识的职责独立：Judge 只裁决知识，PMS 事实不能代替门店政策证据。混合 Task 的知识
部分没有证据时，只对知识部分使用安全边界，已确认 PMS 事实仍须回答。人工优先级同样按 Task
处理：同一个实时问题优先 PMS，独立兄弟 Task 的知识明确转接不被全局压制。同 Task 的知识
转接在 PMS 执行前只延迟、不清空 Judge 事实；PMS 有可用事实时取消该 Task 转接，缺参数、空结果
或查询失败时恢复原 `knowledge_direct_handoff` / `answer_then_handoff`。

日期和目标房型只采用客户当前原话、当前 Task 实体、同 session 已确认定位或 PMS 返回值。
本地只做槽位规范化、纠正覆盖、日期合法性和接口参数校验，不新增第二套意图或自然语言 Judge。
库存必须覆盖同一房型完整入住区间，且请求区间内每天必须存在 `available`；区间外日期不参与
最小可售数计算。差价使用同轮订单与库存快照计算。同轮相同查询可复用，但不得跨轮缓存实时房态。
互不依赖的只读步骤可并发执行；订单绑定、日期绑定、目标房型解析和差价计算仍按依赖顺序执行。

只读 allowlist 不含 `renew` 或其他写 action。可售、可选、会员权益和差价评估只能表述为当前
查询结果，不能表述为已锁房、已换房、已升房、已续住、已延退或已完成收费。
...
多个 `ReplyRequired=true` 的文本任务使用统一协议；单一文本任务允许直接输出自然文本：
...
- 多任务时每个文本 Task 恰好出现一次，顺序与活跃任务清单一致。
- 未知、重复、缺失、空 `taskId/content` 均判为协议错误。
- `coveredFactIds` 只能引用该 Task 的事实，只填写当前回复实际采用的事实，不要求输出无关事实。
- 已声明事实的 `criticalValues` 中，数量、金额、电话、地址、日期、时间、房型名等必须真实
  出现在对应回复文本中。
...
3. 只有协议错误或可重试的模型/网络错误才额外重试一次 Generate；修复提示会
   指出缺失的 Task、非法 Fact 引用或关键值，但不改变问题和证据。
4. 重试前再次检查当前来源消息是否仍允许 AI 回复；员工接管或消息失效时停止。
5. 两次均失败时，知识 Task 可用 Judge 已确认的客户可见事实生成确定性文本；PMS Task
   不倾倒原始查询事实，只追问明确缺失字段或返回安全的重试提示；纯互动使用已有安全短答。
