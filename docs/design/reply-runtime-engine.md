...
同一轮存在多个原子问题时，`text` 必须与本地逐题候选一一对应；候选归属只使用
规范化后的原始 `text`，不能用补全后的 `resolvedText` 或双向子串包含关系抢占
其他问题。每个业务 Task 必须且只能认领一个候选，未认领候选、额外 Task 和完全
重复 Task 都属于 Intent 协议错误。
