...
## 2026-09-24 客户目标继承与验收边界

本节补充下述有界上下文设计，并以真实运行调用链为准：

- JEV 选择所指业务目标，程序同时继承该目标的结构化实体；文本压缩不得丢掉手机号、
  订单定位或已选房型。独立问题与取消不继承旧业务实体，不增加模型阶段。
- 当前明确手机号和房型选择优先于历史实体。更换或否定手机号同时作废依赖旧身份的订单，
  只有当前消息明确提供的订单定位可以保留；“取消旧号码，查询新号码”仍是纠正查询。
- 明确取消终止旧目标恢复；新的独立业务主题阻断旧房型选择的默认继承，但不清除聊天历史。
- 已成功查询并提交答复的客户定位按同一 Task 的完整快照恢复，不跨多次运行拼接字段。
  同一轮包含不同客户定位时不选第一人。快照记录源消息，较旧原话不得反向覆盖它，
  较新原话仍可纠正身份。原始聊天历史不单独猜测当前房型目标。
- PMS 已生成有事实依据的客户答案时，直接提交路径不能再用通用房间说明覆盖它。
  缺少价格依据仍不得报差价，库存不代表锁房，所有 HPMS 写操作保持关闭。

离线/隔离评测分别记录回复检查、业务结果证据、延迟、自然度待复核、真实投递状态。
总分高不能抵消漏答、空回复、额外工具调用或超时。ChannelID=0 的评测只验证真实
模型/PMS/知识链路和本地提交，不验证企微收件，更不等于产品功能全部完成。

## 2026-09-24 有界业务上下文与低延迟回复

JEV 对常见单目标消息合并“目标数量判断”和“业务分类”为一次远程请求；只有 JEV 明确判断
当前消息包含多个目标时，才继续执行边界定位和逐 Task 分类。多问题的 Task、原文来源和顺序
协议保持不变，单问题不再固定承担两次意图模型往返。

上下文使用“当前业务目标快照”，而不是无限拼接聊天历史。快照保留原始目标和最近两条去重
补充，原始消息历史只取最近八条且逐条限长。客户给出手机号、房号、目标房型，或用“上面
那个”“放在哪里”“1501”“不是这个”等短句补充、选择、纠正和拒绝时，JEV 必须明确选择
最近业务 Task；程序只在其同时给出追问、选择、确认、纠正或不满关系时继承原业务路由。
完整的新问题不继承旧主题，独立招呼或闲聊会阻断更早业务目标，避免旧手机号、咖啡或房型
污染后续问题。

JEV 使用历史消息时先去掉 `[历史消息][说话人][时间]` 展示外壳，只保留真实原话。由同一条
客户消息拆出的相邻订单字段，如房型、房号、入住和退房时间，如果均指向同一个订单查询对象，
在进入 ReplyPlan 前合并成一个复合订单目标，避免重复查询和重复发送同一句失败说明；不同
业务路由、不同手机号或不同订单定位仍保持独立 Task。

资源和工单类 Task 可以作为仍在进行的业务目标，明确人工转接 Task 不参与自动继承。客户
对上一答复不满时保留该目标，让系统重新回答真实需求；独立闲聊之后再出现含糊短句时不得
越过闲聊回捞旧目标。

回复阶段优先复用已经完成语义裁决的客户可见结果：完整 Judge 知识答案，以及同时完整的
PMS 与知识多 Task，可以在保持 Task 顺序、关键值、内部字段拦截和 Action Ledger 校验的
前提下直接 Commit，不再经过一次只负责复述的 Generate。存在缺失事实、待调用工具、资源
卡片、人工路由、不安全内容或不完整 Task 时仍走原完整链路。该优化不开放任何 PMS 写操作，
也不把库存、会员或可选房型描述成已经办理完成。

## 2026-09-23 客户目标、事实与回复职责收口

以下为当时设计记录。有关“不预写 AnswerText、所有事实必经 Generate”的描述已被
2026-09-24 的完整事实直接提交路径取代；当前允许有依据的答案投影，但不能覆盖更完整
的客户答案。其余语义、协议和只读安全边界继续适用。
