...
继续使用模型的 `resolvedText`。`evidenceQuery` 仅用于摘取对应检索依据的连续片段，
...
发布状态：以下改动位于 `07bc113`，首轮真实模型验收未通过后已回退，
当前生产仍为 `6108b44`，不能把本节当作已经生效的生产行为。
失败轮的检索改写未召回机器人条目，Judge没有该候选；详情见下述开发记录。
