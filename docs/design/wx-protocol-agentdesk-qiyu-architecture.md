...
- 图片：保存 asset 后调用 vision 模型写入 `mediaText/mediaSummary`，理解成功才进入 AI 上下文。视觉接口偶发失败时保留失败原因，不编造；AI 回复触发只短暂等待最近媒体理解完成，等待窗口约 4 秒，避免客服场景里因为图片理解拖慢整体回复。
...
- AI 对客话术不能暴露“语音识别不准”“图片识别不准”“系统识别”等技术过程。语音已转文字时按客人原话自然回复；确实没听清时只说“这条语音我这边没听清，您方便打字发我一下吗？”
