...
当前架构主链路为：企微员工号协议 SAAS + AgentDesk 网页工作台 + 门店知识库。总部客服统一使用 AgentDesk 网页工作台，不把客户会话转入外部客服系统；新开发、新测试、新部署都以这条链路为准。
...
| 对象 | 职责 | 不做什么 |
| --- | --- | --- |
| 门店企业微信员工号 | 接收客人微信/企微消息，门店员工可在原企微客户端人工回复 | 不保存总部客服凭证，不绕过 AgentDesk 消息表 |
...
如果客户几乎同时发送图片和文本，系统短暂等待最近媒体理解结果，最后一次 AI 运行必须合并最近连续客户消息和已完成的媒体理解；如果媒体尚未理解成功，只能知道“收到过图片/文件”，不能把图片内容当成已识别事实。

### 6.1 媒体展示与 AI 上下文双轨规则

媒体消息在系统内必须同时满足“客服看得见原始媒体”和“AI 稳定读得到理解结果”两条链路：

- 网页端展示仍按原消息类型渲染：图片显示图片预览，语音显示语音转文字卡片，文件显示文件卡片，位置/小程序/GIF 等显示对应结构化卡片，不能为了给 AI 用而把媒体消息改成普通文本。
- `Message.payload` 是媒体理解的唯一持久化载体。图片、语音、文件、视频、GIF 等理解完成后，必须把 `mediaText`、`mediaSummary`、`mediaUnderstandingStatus`、`mediaUnderstandingError` 写回同一条消息的 payload。
- AI 构建上下文时统一调用 `BuildRuntimeMessageTextWithPayload(messageType, content, payload)`。例如网页显示仍是 `[图片] room.jpg`，但 AI 上下文文本应变成：`[图片] room.jpg\n图片内容是：图片里是一间酒店客房...`。
- 理解结果优先级：`mediaText` > `mediaSummary` > `mediaUnderstandingStatus`。有 `mediaText` 时按具体类型写成“图片内容是”“语音内容是”“文件内容是”；只有摘要时写成“图片摘要是”等；失败或处理中只写状态，不得编造内容。
- 媒体理解完成后，后端必须发布 `message.updated` 实时事件，前端只更新该消息的 payload/content，不改变消息气泡类型；同时刷新会话摘要和未读状态。
- 后续会话、上下文压缩、知识进化和人工复盘都读取持久化 payload，不读取临时变量；这样客户隔几条消息后追问“刚才图片里那个是什么”时，AI 仍能看到之前图片的理解结果。
- 如果视觉/文件解析/语音翻译失败，payload 中必须记录失败状态和错误原因；AI 只能基于“无法确认图片/语音/文件内容”继续追问或转人工，不能回复“我看到……”这类假事实。

## 7. AI 回复链路

```mermaid
flowchart TD
  A[客户消息进入 AgentDesk] --> B[落库 去重 echo过滤 session/window]
  B --> C[合并短时间连续客户消息]
  C --> D[补齐媒体上下文 mediaText/mediaSummary]
  D --> E{当前是否人工/AI关闭/无Agent配置}
  E -- 是 --> E1[不触发AI 进入人工或配置告警]
  E -- 否 --> F{轻量意图分类}
  F -- 寒暄/确认/感谢/闲聊 --> G[跳过知识库 自然短句回复]
  F -- 小程序/入住/定位/导航 --> H[走门店默认资源/定位能力]
  F -- 送物/维修/保洁/投诉/高风险 --> I[追问必要字段或转人工 不空口承诺]
  F -- 图片/语音/文件追问 --> J{payload已有理解结果?}
  J -- 有 --> J1[结合媒体理解自然回答]
  J -- 无 --> J2[说明需要补充/稍后再看 不编造]
  F -- 酒店事实/规则/价格/政策/FAQ --> K[只检索当前门店知识库]
  K --> L{命中可信片段?}
  L -- 是 --> M[按知识片段回答 不说根据知识库]
  L -- 否 --> N[只在该类问题上简短兜底/追问/转人工]
  G --> O[写出站消息]
  H --> O
  I --> O
  J1 --> O
  ...
  M --> O
  N --> O
  O --> P[wxwork outbox 按 wework 接口发送]
  P --> Q[echo回调只确认 不重复入库]
```

新的回答链路必须先做意图分类，再决定是否进入知识库。知识库不再是所有消息的第一道门，因此“你好”“确认”“发定位”“发小程序”“帮我看下图片”“送两瓶水”等消息不会因为知识库没命中而被莫名兜底。只有客户询问酒店事实、规则、价格、政策、时效、设施等需要资料支撑的问题，才进入知识库检索；这类问题如果没有命中可信片段，才允许简短兜底、追问或转人工。

`KnowledgeAnswerabilityGate` 的职责只限于“知识类问题的可信资料约束”。它必须跳过以下类型：寒暄确认、运行时动作意图、门店默认资源意图、小程序/定位/导航、真实员工动作、纯媒体消息和媒体追问。跳过 gate 不代表不智能，而是让 Agent 按上下文、工具和自然对话继续处理。

回复速度规则：文本消息只做约 `120ms` 的短 debounce；普通文本不等待媒体理解。只有当前文本明显追问最近图片、语音或文件时，才短等媒体理解，最长约 `900ms`。如果客户先发图片/语音/文件，随后马上补“这个多少钱”“能用吗”“这是什么”“帮我看下”等短问题，应默认理解为围绕刚才媒体追问。这样“早餐几点”“发定位”“我要办入住”“你好”等独立意图不被图片/语音解析拖慢，同时图后追问不会丢媒体上下文。

兜底策略规则：知识库未命中不等于固定套话。只有酒店事实、规则、价格、政策、时效等需要资料支撑的问题，且没有可信资料时，才允许短句确认、追问一个关键细节或转人工。模型可以参考 fallback message，但不能死抄同一句，回复要像前台同事在微信里临场回答。

回复聪明度训练记录见 `docs/design/ai-reply-smartness-training-log.md`。每次调整链路后，至少按该文档 Round 10 的 20 条混合压力样本回放一次，并记录速度、人味、聪明度、媒体关联和兜底控制得分。
...
轻互动不是敷衍回复。感谢类要温和，例如“没事”“不麻烦”；确认类要利落，例如“好，我看着”；结束类要收住，例如“好，那先这样”；表情和玩笑可以轻接，但不能总是“哈哈/收到/可以”，也不要加固定尾巴。
...
- 本地 `healthStatus=login_qrcode` 或 `remote_setup` 只是临时占用，不代表真实登录。超过 30 分钟仍未扫码成功或远程提交的记录，实例池同步时会释放绑定关系，重新显示为空闲可用。

账号管理 UI 规则：账号管理列表本身只展示筛选、刷新、账号列表和已有账号操作，不常驻展示新增方式说明。点击“新增账号”后弹出两栏弹窗：左栏是“总部现场扫码”，右栏是“远程门店自助开户”。这样在会话页抽屉宽度较窄时不会出现卡片文字被压成竖排的问题。

当前真实验证结果：聚合智能账号同步返回 2 个实例，其中 `44406fbf-4fd2-3b9c-a389-71546fa52e0d` 已登录并绑定本地员工号 `吴朝伟`；`7828cf99-9376-30f7-a9ac-01ac6ca87076` 在聚合智能侧 `uin` 为空，旧待扫码记录超过临时占用时效后已被释放，页面显示为空闲可用。若未来给出 100 个实例 ID，系统按上述规则自动筛选空闲项，不使用截图里的 ID 猜测。

### 13.10 账号编辑去工程化与门店员工自助配置

2026-06-29 的账号管理改造把“协议实例工程字段”和“门店日常运营字段”彻底拆开。会话页左侧账号管理仍可按 `guid` 搜索，但账号编辑抽屉不再展示 `guid / channelId / employeeUserId / employeeAvatar URL / notifyUrl / proxy / bridgeId / staffUserIds` 等字段。

原因：这些字段属于协议运行时或系统内部绑定，不应该让门店员工、客服组长在日常编辑里理解和填写。`employeeUserId` 是协议登录后同步到的当前企微员工号 username，主要用于判断平台 echo 和自己发出的消息；`guid` 是上游 XBot 实例 ID；`notifyUrl/proxy/bridgeId` 是协议客户端配置；`staffUserIds` 后续会被正式的门店员工系统账号/客服组绑定关系替代。

账号编辑抽屉只保留以下面向运营的字段：

| 区域 | 可编辑内容 | 说明 |
| --- | --- | --- |
| 员工号资料 | 员工号展示名、绑定门店 | 员工号头像和 UserID 由协议资料同步；展示名可人工修正乱码或门店简称 |
| 门店定位 | 地址、导航名称、经纬度、坐标来源、一键浏览器定位 | 客户询问“酒店在哪/怎么去/发定位”时使用这里的门店变量，不写死天安门或单个账号坐标 |
| 资源绑定 | 只展示说明 | 小程序跟随企业/品牌统一绑定；知识库、模型、提示词、技能在账号行“智能客服配置”维护 |
| 人工接待路由 | 服务时间、门店群通知开关、门店群选择、@成员选择、总部兜底、人工超时 | 转人工时按时间段判断走门店群还是总部网页端 |
| 自动化开关 | AI 托管回复、自动通过好友申请、好友备注模板 | AI 开关只控制托管状态，不承载提示词/知识库 |
| 状态与备注 | 启用状态、备注 | 运维备注，不参与协议字段猜测 |

提交规则：前端不把被隐藏字段置空。保存时从当前账号详情 `context.item` 带回原 `guid/channelId/employeeUserId/employeeAvatar/notifyUrl/proxy/bridgeId/staffUserIds/knowledgeBaseId/aiAgentId/context` 等值，仅提交用户可编辑的运营字段变化。这样既不暴露工程字段，也不会破坏旧实例、回调和 outbox。

门店员工系统账号与协议实例登录必须区分：

1. 协议实例登录：用于把某个酒店员工企业微信号登录到上游协议实例，拿到 `guid` 并接收/发送客户消息。它由“新增账号”弹窗左侧现场扫码或右侧远程自助开户链接完成。
2. 门店员工系统登录：用于门店同事登录 AgentDesk 自己的门店页面，维护门店资料、服务时间、选择门店群、选择 @ 成员、查看本门店转人工提醒。它应走企业微信 OAuth/扫码登录进入 AgentDesk，创建/绑定系统内部用户和门店权限，不等同于协议实例登录。
...
“账号管理”列表顶部只保留刷新和新增按钮，不常驻展示新增方式卡片。点击“新增账号”后弹出两栏：

- 总部现场扫码：总部有人在现场时使用。系统从实例池自动认领空闲 `guid`，生成协议登录二维码，扫码成功后同步员工号资料和回调。
- 远程门店自助开户：门店离总部较远时使用。总部复制链接发给门店负责人，门店负责人打开页面后完成协议扫码、填写门店名称/地址/坐标/服务时间，并可一键获取当前位置。

远程门店页面后续还需要补齐“选择门店群/选择 @ 成员/搜索知识库绑定/门店员工系统账号绑定”。这些属于门店自助运营配置，不应回到总部账号编辑里用文本框填 ID。

会话工作台里的入口必须保持单一：会话左栏主操作区保留“新增账号”，点开后就是左右两栏弹窗；“账号设置/账号管理”只负责编辑、删除、更换登录员工号、智能客服配置和托管策略，不再出现第二个“新增账号”按钮，避免总部运营误以为有两套新增流程。

### 13.13 账号操作菜单收敛与门店员工角色

账号管理行操作只保留产品动作，不再暴露协议运维动作。以下动作从前端菜单隐藏，仅保留后端接口供系统自动调用或高级运维使用：设置回调、获取登录二维码、同步账号资料、同步好友申请、恢复实例、停止实例、复制回调地址。

日常真正需要给客服组长/总部运营看到的是：

1. 智能客服配置：进入当前员工号独立智能客服配置，维护模型、提示词、知识库和技能。
2. 更换登录员工号：当当前企业微信员工号被封、离职、不可用时，系统先调用协议 `logout` 让该实例退出当前企微登录，再调用登录二维码接口生成重新扫码材料，让门店重新扫码登录另一个企业微信员工号。
3. 删除：删除本地员工号配置。删除前必须确认不会误删仍在使用的门店会话链路。

企业微信登录 AgentDesk 的系统账号默认角色为 `store_staff`（门店员工）。首次通过企业微信 OAuth/扫码登录进入系统时，后端创建 `User + UserIdentity` 后自动分配该角色。`store_staff` 不是协议实例账号，不等于 `WxWorkProtocolInstance`；它代表门店同事在 AgentDesk 里的系统身份。

`store_staff` 的导航与权限必须最小化：默认只进入 `/dashboard/store-workbench` 门店工作台。该页面采用自适应设计，移动端单列，桌面端双列，展示门店资料、企业微信员工号状态、服务时间、通知群、智能客服状态和待处理提醒。它不显示 GUID、回调地址、代理、Bridge、实例池、渠道管理等运维能力。

后续实现门店员工自助配置时，必须在这个页面内补齐：搜索并绑定门店、搜索并绑定门店知识库、选择门店群、选择 @ 成员、一键获取坐标、查看本门店转人工提醒。所有选择都应走系统接口和协议接口返回的真实列表，不允许要求门店员工手填内部 ID。

账号编辑里必须直接展示“门店托管模式”下拉框，选项为全托管、半托管、非托管。保存时写入 `StoreStaffBinding.managedMode`，不是只写实例展示字段：

- 全托管：不看服务时间，不发门店群，转人工只进入总部网页端。
- 半托管：按服务时间在门店群与总部网页端之间切换。
- 非托管：不走总部网页端，转人工只发绑定门店群。
