...
- 图文/语音追问场景增加媒体等待：如果当前文本消息之前 8 秒内有客户发来的图片、语音或附件，且媒体理解状态仍未完成，AI 最多等待约 8 秒；等待期间如果客户继续发新消息，当前触发取消，交给最新消息处理。
...
定位不是模型自由生成的答案，而是门店主数据驱动的协议发送动作。每个门店需要在后台维护：

- `address`：门店详细地址。
- `navigationName`：地图里展示的导航名称，例如“丽斯未来酒店杭州西湖店”。
- `longitude/latitude`：经纬度，统一使用协议文档要求的坐标系和字段格式。
- `phone`：门店联系电话，可用于文字补充。
- `mapProvider`：坐标来源，例如高德、腾讯、百度，用于后续坐标转换审计。
...
一百多家门店时，不建议在每个员工号里重复手填坐标。正确做法是批量维护门店表，再让多个员工号绑定门店。若一家门店有多个员工号，它们共享同一套门店坐标；若同一客户联系不同门店员工号，定位按当前 `guid` 对应门店发送。门店没有坐标时，系统不能让 AI 编坐标，只能回复“我这边暂时没有定位信息，先把门店地址发您，并通知同事补充定位”。
