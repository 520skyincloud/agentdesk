...
排班继续复用综合客服组、小组和多条 `AgentTeamSchedule`，支持日历、日期范围、一天多个不重叠时段、跨午夜、临时替班和请假排除。详细约束以 `docs/design/agent-team-squad-scheduling.md` 为准。

分配逻辑：

1. 根据会话 `storeId/wxWorkInstanceId` 找可服务客服组。
2. 根据当前排班、小组、替班/请假找计划值班成员。
3. 再校验账号、权限、自动接单、实际 Presence、服务范围和最大并发。
4. 规则模式按实时压力、本班工作量和稳定轮转派发；人工模式保留在组长编排池。
5. 无人可接时保留在原综合组，达到 Queue SLA 后通知组长或公司主管。

同一综合组班次不能重叠；零人覆盖禁止保存，一人覆盖提示单点风险。模型选择客服已退役，当前只存在人工和确定性规则派单。
...
保留旧 `startTime/endTime` 兼容；批量排班 UI 已支持多个时间段。规则派单只从当前唯一有效班次中选择候选，并叠加小组、替班、请假和实际 Presence。
...
- integration 已具备 Tenant、权限、客服组/小组/排班、会话消息、人工/规则派单、会话监控基础和租户测试数据；模型选择客服已退役。
