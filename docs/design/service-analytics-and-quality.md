...
| 人工、规则自动派单 | 已提交并通过全量回归 | 人工编排与确定性公平规则共用现有派单状态机；模型派单已退役 |
...
| 丽斯未来测试租户与仿真数据 | 运营仿真已补齐并通过幂等/清理/在线保活测试 | 已生成服务轮次、响应分段、Presence、人工质检、评价和完整派单证据；12 名仿真客服初始均在线，运行 `keepalive` 时每分钟刷新且不触碰真实客服 |
...
静态 Seed 只负责建立可重复的 12 条 Presence 基线，不能用未来时间伪造永久在线。需要持续观察自动派单时，另行启动仿真保活进程：

```text
go run ./cmd/customer_audit_seed --config config/config.yaml --action keepalive --batch customer-audit-v1
```

该进程默认每分钟把丽斯未来测试租户的 12 个固定测试客服刷新为 `online`，停止进程后生产规则仍会在 3 分钟新鲜度到期后将其判为离线。作用范围同时受固定租户注册身份、当前 batch 的 `TEST_SEED` 标记、固定测试用户名和客服档案标记约束；不会把排班等同在线，也不会修改真实租户、生产 Presence 判定、AI 回复、模型调用或计费。
...
### 批次 8：丽斯未来测试客服在线保活（已实现并通过真实运行验收）

- Seed 创建的 12 名测试客服初始统一为 `online`，不再混入 busy/break 状态影响规则派单仿真。
- `customer_audit_seed --action keepalive` 每分钟刷新同一批 12 条合成 Presence；缺失行会补建，已结束行会复用，重复合成行和其他活动行会收口为每人一个活动时段。
- 保活只识别丽斯未来固定测试租户、当前 batch 标记及 12 个固定客服账号；真实客服仍必须依赖 WebSocket/工作台心跳，排班本身不代表在线。
- 本批没有 model、migration、DTO、enum、接口、WebSocket、权限、AI、Token 或计费变化；定向测试覆盖全员初始在线、过期/休息恢复、缺失补建、重复刷新幂等和无关账号不变。
- 当前 8084 同库保活已连续刷新，派单页从 0 名恢复为 12 名可接单客服，12 人均显示“在线且在班，可自动接单”；原 9 条规则积压已由后端补偿扫描派完，当前待派为 0，页面控制台无 error/warn。
