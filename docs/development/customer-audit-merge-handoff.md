...
3. “全部账号”会话视图中保留当前列表，同时高亮会话来源企微账号。
...
双向绑定前端通过 `pnpm typecheck` 和目标 ESLint 后方可提交；页面视觉验证记录保留在本地 `.codex/audits/agent-team-binding/`，截图不进入 PR。
...
- 仿真数据和会话导航仍待拆分提交；协同规则、后端共享契约和前端双向绑定已按职责拆分。
