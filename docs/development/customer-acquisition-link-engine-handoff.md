...
2026-08-01 修复员工号实例页面只对非在线实例显示“扫码重新登录”的前端门禁。登录能力、
二维码接口、三秒轮询、状态 `10` 确认码提交和成功后资料同步均仍在原运行链，本次只恢复
现有实例的操作入口，没有新增第二套登录流程。

当前规则为：实例未删除、仍在有效期、协议允许登录、GUID 非空且操作者拥有渠道更新权限
时，无论当前为在线或离线均可发起扫码重新登录。在线实例获取二维码成功后继续保持本地
`online`，直到协议真实回调或登录检查产生新的运行事实，避免用户尚未完成扫码时把当前
接待链路误判为离线；离线等非在线实例仍进入 `login_qrcode` 状态。

本次不修改协议请求契约：取码请求仍只有 `guid` 和布尔 `verify_login=false`，检查及状态
`10` 验证码接口保持原样。没有新增 model、migration、DTO、enum、权限、路由或 WebSocket
契约，也不影响小程序、AI 回复、企业微信第三方应用授权和消息协议。

验证结果：

```text
员工号登录聚焦 service 测试                     通过
internal/services 全量测试                      通过
dashboard handler 全量测试                      通过
go test ./... -count=1                          通过
前端 Node 契约测试                              166/166 通过
pnpm typecheck                                  通过
pnpm build                                      通过，49 个页面
git diff --check                                通过
```

并行分支无需 rebase 前置或契约提交；本次只改变当前集成分支上的页面可见条件和既有 service
状态写入，不吸收或覆盖 `codex/ai-billing` 的模型、密钥、计费及标签实现。建议继续以当前统一
集成分支为发布主体。回滚只需切换到部署前应用镜像并强制重建 `agent-desk`，无数据库回滚；
回滚后有效在线实例会再次看不到“扫码重新登录”。

## 18. 替换草稿消息回调诊断与恢复入口

2026-08-03 排查生产环境客户消息未进入会话列表。公网员工号回调、短回调 URL、渠道鉴权
和新实例登录均正常；真实 `11010` 消息事件已经到达 AgentDesk，但来源实例属于尚未完成
邮箱/远程设置验证的替换草稿。按照当前门店身份与会话连续性规则，草稿不能在旧实例归档
前接管消息，因此处理器拒绝落库。生产门店员工系统账号尚未登记邮箱，现有替换流程无法完成
邮箱验证，这是当前实例仍未激活的直接操作阻塞。

本次没有放宽替换激活条件。员工号回调对单条和批量消息事件先识别未验证替换草稿，写入
`failed` 同步审计并以 `409`、安全阶段码 `replacement_pending_verification` 拒绝；不再留下
无法区分原因的 `pending` 记录。结构化日志和错误原因不包含 GUID、员工号、客户标识、
conversation_id、消息原文或任何渠道密钥。

员工号实例页将替换草稿显示为“待完成更换”，隐藏普通“扫码重新登录”和再次创建替换的
操作，新增“继续完成更换”入口。扫码弹窗若操作对象是替换草稿，只确认扫码登录完成并引导
继续验证，不再宣称实例已经接管消息。普通在线、离线实例的原扫码重登能力保持不变。

数据恢复边界：先由管理员为绑定的门店员工系统账号登记真实邮箱，再通过现有替换链接完成
邮箱验证码与远程设置提交；服务会在原事务中激活新实例、归档旧实例并迁移到店运行关系。
不得直接修改 `remote_setup_submitted_at` 或伪造验证码。激活后通过协议消息补偿能力同步验证
期间遗漏的消息，再以新客户消息确认回调 `200`、同步日志 `success` 和会话落库。

本次改动没有 model、migration、后端 DTO、枚举、路由或 WebSocket 变化；前端仅补齐后端
既有替换关系字段的类型定义。与 `codex/ai-billing` 存在历史同文件修改，但当前统一集成分支
已经包含对应模型、知识与计费能力，本次无需先行 rebase；后续合并应以统一集成分支版本为准。
