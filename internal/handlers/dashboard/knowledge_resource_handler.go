...
func KnowledgeResourceAnyList(ctx *gin.Context) {
	...
	for index := range groups {
		...
		results = append(results, builders.BuildKnowledgeResourceGroup(group, services.KnowledgeResourceService.FindItems(group.ID, group.TenantID)))
	}
...
func KnowledgeResourcePostSync(ctx *gin.Context) {
	...
	httpx.WriteJSON(ctx, builders.BuildKnowledgeResourceGroup(group, services.KnowledgeResourceService.FindItems(group.ID, group.TenantID)))
}
