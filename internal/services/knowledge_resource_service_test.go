...
func TestResolveKnowledgeResourcesStaysWithinStoreAndKnowledgeScopeAcrossAccountReplacement(t *testing.T) {
	...
	for _, instance := range []models.WxWorkProtocolInstance{
		{ID: 41, Guid: "old-account", CompanyID: 11, StoreID: 7, Status: enums.StatusOk},
		{ID: 42, Guid: "replacement-account", CompanyID: 11, StoreID: 7, Status: enums.StatusOk},
		{ID: 43, Guid: "other-store", CompanyID: 11, StoreID: 8, Status: enums.StatusOk},
	} {
	...
	resources := KnowledgeResourceService.ResolveForRuntime(41, 11, 21, sources)
	if len(resources) != 1 || resources[0].AssetID != asset.AssetID {
		t.Fatalf("expected exactly scoped resource, got %+v", resources)
	}
	resources = KnowledgeResourceService.ResolveForRuntime(42, 11, 999, sources)
	if len(resources) != 1 || resources[0].AssetID != asset.AssetID {
	...
	for _, scope := range []struct {
		...
		{name: "other store", instance: 43, company: 11, profile: 21, sourceRef: sources},
		{name: "other company", instance: 41, company: 12, profile: 21, sourceRef: sources},
		{name: "other source", instance: 41, company: 11, profile: 21, sourceRef: []KnowledgeResourceSourceRef{{KnowledgeBaseID: 31, SourceRecordID: "fastgpt-record-002"}}},
	} {
		t.Run(scope.name, func(t *testing.T) {
			if got := KnowledgeResourceService.ResolveForRuntime(scope.instance, scope.company, scope.profile, scope.sourceRef); len(got) != 0 {
