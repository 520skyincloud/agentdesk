...
func (s *knowledgeResourceService) SyncFastGPTResources(ctx context.Context, req request.SyncKnowledgeResourceGroupRequest, operator *dto.AuthPrincipal) (*models.KnowledgeResourceGroup, error) {
	...
	instance := WxWorkProtocolInstanceService.Get(req.WxWorkInstanceID)
	...
	knowledgeBase := KnowledgeBaseService.Get(req.KnowledgeBaseID)
...
func (s *knowledgeResourceService) FindItems(groupID int64) []models.KnowledgeResourceItem {
	if groupID <= 0 {
		return nil
	}
	return repositories.KnowledgeResourceItemRepository.FindByGroupID(sqls.DB(), groupID)
}
...
func (s *knowledgeResourceService) DeleteGroup(id int64, operator *dto.AuthPrincipal) error {
	...
	group := repositories.KnowledgeResourceGroupRepository.Get(sqls.DB(), id)
	if group == nil {
	...
	items := repositories.KnowledgeResourceItemRepository.FindByGroupID(sqls.DB(), group.ID)
	if err := sqls.WithTransaction(func(tx *sqls.TxContext) error {
		if err := repositories.KnowledgeResourceItemRepository.DeleteByGroupID(tx.Tx, group.ID); err != nil {
			return err
		}
		return repositories.KnowledgeResourceGroupRepository.Delete(tx.Tx, group.ID)
	}); err != nil {
		return err
	}
...
func (s *knowledgeResourceService) ResolveForRuntime(wxWorkInstanceID, companyID, _ int64, sources []KnowledgeResourceSourceRef) []RuntimeKnowledgeResource {
	if wxWorkInstanceID <= 0 || len(sources) == 0 || sqls.DB() == nil {
		return nil
	}
	...
	for _, source := range sources {
		...
		group := repositories.KnowledgeResourceGroupRepository.Take(sqls.DB(),
			"company_id = ? AND store_id = ? AND knowledge_base_id = ? AND source_provider = ? AND source_record_id = ? AND status = ?",
			companyID,
			storeID,
		...
		if group == nil {
			group = repositories.KnowledgeResourceGroupRepository.Take(sqls.DB(),
				"knowledge_base_id = ? AND wx_work_instance_id = ? AND source_provider = ? AND source_record_id = ? AND status = ?",
				source.KnowledgeBaseID,
				wxWorkInstanceID,
				knowledgeResourceProviderFastGPT,
				strings.TrimSpace(source.SourceRecordID),
		...
		for _, item := range repositories.KnowledgeResourceItemRepository.FindByGroupID(sqls.DB(), group.ID) {
...
func (s *knowledgeResourceService) persistSyncedResources(instance *models.WxWorkProtocolInstance, knowledgeBase *models.KnowledgeBase, source rag.FastGPTSyncSource, images []downloadedKnowledgeResourceImage, operator *dto.AuthPrincipal) (*models.KnowledgeResourceGroup, error) {
	...
	existing := repositories.KnowledgeResourceGroupRepository.Take(sqls.DB(),
		"company_id = ? AND store_id = ? AND knowledge_base_id = ? AND source_provider = ? AND source_record_id = ?",
		instance.CompanyID,
		instance.StoreID,
		knowledgeBase.ID,
		knowledgeResourceProviderFastGPT,
	...
	if existing != nil {
		oldItems = repositories.KnowledgeResourceItemRepository.FindByGroupID(sqls.DB(), existing.ID)
	}
	...
	for _, imageItem := range images {
		...
		assetID := ""
		if oldItem, ok := reusable[key]; ok && AssetService.GetByAssetID(oldItem.AssetID) != nil {
	...
	err := sqls.WithTransaction(func(tx *sqls.TxContext) error {
		if group == nil {
			...
			if err := repositories.KnowledgeResourceGroupRepository.Updates(tx.Tx, group.ID, map[string]any{
				"title":            source.Title,
				"description":      source.Description,
	...
	return repositories.KnowledgeResourceGroupRepository.Get(sqls.DB(), group.ID), nil
}
...
func (s *knowledgeResourceService) cleanupUnreferencedAssets(assetIDs []string, operator *dto.AuthPrincipal) {
	...
	for _, assetID := range assetIDs {
		...
		seen[assetID] = true
		if len(repositories.KnowledgeResourceItemRepository.FindByAssetID(sqls.DB(), assetID)) > 0 {
			continue
		}
		if asset := AssetService.GetByAssetID(assetID); asset != nil {
