...
func (s *fastGPTUsageSyncService) SyncKnowledgeBase(knowledgeBaseID int64) error {
	...
	if knowledgeBase == nil || knowledgeBase.ConnectionID != fastgptapi.ManagedConnectionID || knowledgeBase.StoreID <= 0 || strings.TrimSpace(knowledgeBase.DatasetID) == "" {
		return fmt.Errorf("managed FastGPT knowledge base is unavailable")
	}
	tenant := repositories.FastGPTStoreTenantRepository.GetByStoreID(sqls.DB(), knowledgeBase.StoreID)
	...
	state := repositories.FastGPTUsageSyncStateRepository.GetByKnowledgeBaseID(sqls.DB(), knowledgeBase.ID)
	if state == nil {
		now := time.Now()
...
func (s *fastGPTUsageSyncService) syncProfileSnapshot(knowledgeBase *models.KnowledgeBase, profile *fastgptapi.DatasetProfileSnapshot) {
	...
	now := time.Now()
	_ = repositories.KnowledgeBaseRepository.Updates(sqls.DB(), knowledgeBase.ID, map[string]any{
