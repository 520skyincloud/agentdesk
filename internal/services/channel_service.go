...
func (s *channelService) ParseWxWorkCLIChannelConfig(raw string) (*dto.WxWorkCLIChannelConfig, error) {
	raw = strings.TrimSpace(raw)
	cfg := &dto.WxWorkCLIChannelConfig{
		DefaultChatType: 1,
	}
	if raw != "" {
		if err := json.Unmarshal([]byte(raw), cfg); err != nil {
	...
	if cfg.DefaultChatType != 1 && cfg.DefaultChatType != 2 {
		cfg.DefaultChatType = 1
	}
	return cfg, nil
}

func (s *channelService) ParseWxWorkProtocolChannelConfig(raw string) (*dto.WxWorkProtocolChannelConfig, error) {
	raw = strings.TrimSpace(raw)
...
func (s *channelService) buildChannelModel(id int64, req request.CreateChannelRequest) (*models.Channel, error) {
	...
	case enums.ChannelTypeWxWorkCLI:
		...
		cfg, err := s.ParseWxWorkCLIChannelConfig(configJSON)
		if err != nil {
			return nil, errorsx.InvalidParam("企业微信CLI渠道配置不合法")
		}
		if cfg.BridgeToken == "" {
			secret, err := generateUserTokenSecret()
			if err != nil {
	...
	case enums.ChannelTypeWxWorkProtocol:
		...
		cfg, err := s.ParseWxWorkProtocolChannelConfig(configJSON)
		if err != nil {
