...
func TestTenantIntegrityAuditPassesCleanTwoTenantFixture(t *testing.T) {
	...
	if report.RequiredTables != 122 || report.ConfiguredRelations != 317 {
		t.Fatalf("audit schema coverage = %d tables/%d relations, want 122/317", report.RequiredTables, report.ConfiguredRelations)
	}
