...
var dashboardZhCN = map[string]string{
	...
	"alert.aiNoKnowledge.title":         "接待策略未绑定知识库",
	"alert.aiNoKnowledge.description":   "部分启用中的接待策略尚未绑定知识库，回答质量可能不稳定。",
...
var dashboardEnUS = map[string]string{
	...
	"alert.aiNoKnowledge.title":         "Reception strategies are missing knowledge bases",
	"alert.aiNoKnowledge.description":   "Some enabled reception strategies are not linked to a knowledge base yet, which may reduce answer quality.",
