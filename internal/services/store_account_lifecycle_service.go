...
func (s *storeAccountLifecycleService) CompleteRemoteSetup(instance *models.WxWorkProtocolInstance, req request.UpdateWxWorkProtocolRemoteSetupRequest) (*models.WxWorkProtocolInstance, error) {
	...
	err = sqls.WithTransaction(func(ctx *sqls.TxContext) error {
		...
		if current.ReplacesInstanceID > 0 {
			...
			binding := repositories.StoreStaffBindingRepository.Take(ctx.Tx, "tenant_id = ? AND store_id = ? AND status <> ?", current.TenantID, replaced.StoreID, enums.StatusDeleted)
			if binding == nil || binding.UserID <= 0 {
				return errorsx.InvalidParam("原门店尚未绑定主邮箱账号")
			}
			owner := repositories.UserRepository.GetInTenant(ctx.Tx, binding.UserID, current.TenantID)
...
func (s *storeAccountLifecycleService) ensureStableStore(db *gorm.DB, instance *models.WxWorkProtocolInstance, requestedStoreID int64, storeName string) (*models.Store, error) {
	...
	if storeID > 0 {
		store := repositories.StoreRepository.GetInTenant(db, storeID, instance.TenantID)
		...
		if err := repositories.StoreRepository.UpdatesInTenant(db, store.ID, instance.TenantID, updates); err != nil {
			return nil, err
		}
...
func (s *storeAccountLifecycleService) ensurePrimaryStoreUser(db *gorm.DB, store *models.Store, email, storeName string) (*models.User, error) {
	...
	if user != nil {
		if user.TenantID != store.TenantID {
			return nil, errorsx.InvalidParam("该邮箱已属于其他接入公司")
		}
		if user.Status != enums.StatusOk || user.DeletedAt != nil {
			return nil, errorsx.InvalidParam("该邮箱绑定的系统账号不可用")
		}
		if binding := repositories.StoreStaffBindingRepository.Take(db, "tenant_id = ? AND user_id = ? AND store_id <> ? AND status <> ?", store.TenantID, user.ID, store.ID, enums.StatusDeleted); binding != nil {
			return nil, errorsx.InvalidParam("该邮箱已绑定其他门店")
		}
		now := time.Now()
		if err := repositories.UserRepository.UpdatesInTenant(db, user.ID, store.TenantID, map[string]any{"email_verified_at": now, "updated_at": now, "update_user_id": user.ID, "update_user_name": user.Username}); err != nil {
			return nil, err
		}
...
func (s *storeAccountLifecycleService) ensureStoreStaffRole(db *gorm.DB, user *models.User) error {
	...
	return UserService.ensureSystemUserRoleDB(db, user.ID, role.ID, &dto.AuthPrincipal{
		UserID: user.ID, TenantID: user.TenantID, ActiveTenantID: user.TenantID,
...
func (s *storeAccountLifecycleService) ensureStoreStaffBinding(db *gorm.DB, instance *models.WxWorkProtocolInstance, store *models.Store, user *models.User, req request.UpdateWxWorkProtocolRemoteSetupRequest) (*models.StoreStaffBinding, error) {
	now := time.Now()
	if binding := repositories.StoreStaffBindingRepository.Take(db, "tenant_id = ? AND store_id = ? AND status <> ?", store.TenantID, store.ID, enums.StatusDeleted); binding != nil {
		if binding.UserID > 0 && binding.UserID != user.ID {
			return nil, errorsx.InvalidParam("该门店已绑定其他主邮箱账号")
		}
		if err := repositories.StoreStaffBindingRepository.UpdatesInTenant(db, binding.ID, store.TenantID, map[string]any{
			"user_id":                    user.ID,
			"company_id":                 store.CompanyID,
