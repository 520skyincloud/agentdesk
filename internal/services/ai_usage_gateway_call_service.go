...
func (s *aiUsageGatewayCallService) reconcileOne(client *newapi.Client, item *models.AIUsageGatewayCall) error {
	...
	if err != nil {
		_ = repositories.AIUsageGatewayCallRepository.UpdatesInTenant(sqls.DB(), item.ID, item.TenantID, map[string]any{
	...
	return repositories.AIUsageGatewayCallRepository.UpdatesInTenant(sqls.DB(), item.ID, item.TenantID, map[string]any{
