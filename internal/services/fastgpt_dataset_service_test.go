...
func TestFastGPTDatasetJobsAllowMultipleKnowledgeBasesForOneStore(t *testing.T) {
	...
	store := &models.Store{TenantID: 101, Name: "测试门店", Status: enums.StatusOk}
	if err := db.Create(store).Error; err != nil {
		t.Fatalf("create store: %v", err)
	}
...
func TestFastGPTKnowledgeBaseActivationOnlyChangesSelectedInstance(t *testing.T) {
	...
	const tenantID int64 = 101
	store := &models.Store{TenantID: tenantID, Name: "测试门店", KnowledgeBaseID: 101, Status: enums.StatusOk}
	if err := db.Create(store).Error; err != nil {
		t.Fatalf("create store: %v", err)
	}
	active := &models.KnowledgeBase{TenantID: tenantID, StoreID: store.ID, DatasetID: "dataset-active", Name: "当前库", Status: enums.StatusOk}
	candidate := &models.KnowledgeBase{TenantID: tenantID, StoreID: store.ID, DatasetID: "dataset-next", Name: "新库", Status: enums.StatusOk}
