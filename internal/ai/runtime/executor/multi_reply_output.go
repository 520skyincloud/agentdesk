...
func buildMultiReplyOutputInstruction(plan callbacks.ReplyPlanTraceData, requireStructured bool) string {
	...
	for _, group := range groups {
		...
		if !group.ExternalProxyAction && len(group.Facts) > 0 {
			part.CoveredFactIDs = []string{group.Facts[0].FactID}
		}
...
func renderLockedReplyFacts(facts []replyFactRequirement) string {
	...
	for _, fact := range facts {
		if statement := strings.TrimSpace(fact.Statement); statement != "" && !seen[statement] {
			statements = append(statements, statement)
			seen[statement] = true
		}
	}
	return joinGeneratedReplyFactStatements(statements)
}
...
func buildTextReplyTaskGroups(plan callbacks.ReplyPlanTraceData) []textReplyTaskGroup {
	...
	for _, task := range plan.TaskPlans {
		...
		// Judge and PMS select evidence; the final response model owns wording
		// whenever evidence exists. Only factless server-authored control text is
		// locked, because there is nothing semantic for Generate to reinterpret.
		evidenceLocked := task.AnswerText != nil && len(task.SupportedFacts) == 0
	...
	for index := range groups {
		...
		for _, fact := range groups[index].Facts {
			owned := false
			for _, sibling := range groups {
				if sibling.ExternalProxyAction || sibling.SelectedLayer != groups[index].SelectedLayer {
					continue
				}
				for _, siblingFact := range sibling.Facts {
					if strings.TrimSpace(siblingFact.Statement) == strings.TrimSpace(fact.Statement) {
						owned = true
						break
					}
				}
