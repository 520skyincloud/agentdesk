...
混合 FAQ 中能独立回答当前问题的次要事实可以使用，不能仅因 FAQ 标题不同就排除。原问题的物品或主题不自动成为后续独立操作建议的限制条件；建议明确提供当前目标的办理渠道或方法时，可以直接采用该建议，不复制前面的无关否定结论，也不要求额外的“酒店是否允许”证明。只有建议自身或问答上下文明示的条件才约束其使用，不能凭标题添加条件；如果明确以“仅限、必须、如果……才……”限制方案，仍须保留并核对，不能把“在某条件下可采用的办法”改成无条件政策。无关对象的提供情况、未触发的条件式替代建议或仅有关键词重合的背景不能一起带入。task.entities 有多个明确实体时，direct_combined 的必要同层证据必须逐一覆盖；不同任务分别选择，不能串用另一个任务的结论。
...
适用知识的可回答性不等于所有细节都已确认：知识明确提供与客户当前目标直接对应的服务、专用设施、领取方式或办理入口时，先保留这份可用答复；客户实际问到的方面尚不完整时可以判 partial，不能因此把整条知识判 insufficient。没有询问且不影响答案成立的配送、费用、时长等细节，不加入必要维度或 missingAspects；已经回答本题全部要求时判 direct_single/direct_combined。answerText 只回答选中知识已经写明且本题需要的内容，不补出“可以、能送到、已安排”等原文没有确认的结论，也不自动追加未知说明或联系人工建议。partial 不是保留所有相关背景的许可。仅同属一个话题、不能回应当前目标的地址、名单或背景仍应排除；设施已经故障、方案已被拒绝或客户坚持必须现场执行时，不能用设施存在性充当解决办法。
...
- 专用设施只有与当前目标直接对应且适用时才是可用途径，不能把其他用途的设施或商家名单当作自助方案，也不能从设施存在推断配送范围、代点餐或已执行；事实、条件与知识层仍不得跨对象拼接。
...
最小完整答案规则：在同一次裁决中依次确定当前要求、适用事实、完整性后，先形成当前任务的 answerText，再从该 answerText 提取 supportedFacts，最后原样摘取 criticalValues，JSON 也按此顺序输出。不能先复制候选答案，再为其中的无关内容寻找理由。必要的事实、适用条件和操作方法不能遗漏；背景介绍、重复总结、礼貌话、未被客户询问的路线/时长/价格/延伸建议不得加入。普通问题1至2句，流程保留必要的2至3个简短步骤，不截断必要信息。
...
事实维度限制的是答复外推，不是否决已有知识：服务或专用设施的存在性事实可以保留为同目标的适用答复，但不能扩写未确认的配送范围、使用条件或执行结果；地点名称不能生成距离、步行时间或路线。客户询问了仍然适用但未被证据确认的维度时，保留可用知识、判 partial 并将缺失记入 missingAspects，而不是清空答案。只有不存在适用于当前需求的知识答复时才判 insufficient。
