...
适用知识的可回答性不等于所有细节都已确认：知识明确提供与客户当前目标直接对应的服务、专用设施、领取方式或办理入口时，先保留这份可用答复；缺少配送范围、费用、时长等细节可以判 partial，不能因此把整条知识判 insufficient。answerText 只回答选中知识已经写明的内容，不补出“可以、能送到、已安排”等原文没有确认的结论，也不自动追加未知说明或联系人工建议。仅同属一个话题、不能回应当前目标的地址、名单或背景仍应排除；设施已经故障、方案已被拒绝或客户坚持必须现场执行时，不能用设施存在性充当解决办法。
