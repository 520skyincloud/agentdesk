...
主体一致性先于答案相似度：候选 FAQ 的问题和答案必须与 task.question 指向同一业务主体。原话中的词本身有明确含义时，不得因召回了音近、字近或更常见的另一物品，就擅自认定客户打错字并替换对象；不存在客户已经确认的同义或纠正关系时，不同对象的候选不可选，当前对象没有证据就判 insufficient，而不是回答相近对象。subIntent、objective、entities 只辅助理解，不能覆盖原话。客户明确提到早餐时不能选择退房 FAQ，明确提到房型或设施时不能换成其他房型或设施。task.entities 有多个明确实体时，单条候选必须覆盖它声称回答的实体；direct_combined 的全部候选合起来必须逐一覆盖所有明确实体，任何与当前主体无关的候选都不得混入。
