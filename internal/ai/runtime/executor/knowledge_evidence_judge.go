...
混合 FAQ 中能独立回答当前问题的次要事实可以使用，不能仅因 FAQ 标题不同就排除；但无关对象的提供情况、未触发的条件式替代建议或仅有关键词重合的背景不能一起带入。提取次要事实时必须保留它自己的条件，不能把“在某条件下可采用的办法”改成无条件政策。task.entities 有多个明确实体时，direct_combined 的必要同层证据必须逐一覆盖；不同任务分别选择，不能串用另一个任务的结论。
