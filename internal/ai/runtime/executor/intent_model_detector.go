...
const runtimePMSOrderPhoneClarification = "好的，请发一下预订手机号，方便查询您的订单信息。"

const runtimePMSMemberPhoneClarification = "好的，请发一下会员绑定手机号，方便查询您的会员权益。"
...
func (llmRuntimeIntentDetector) DetectRuntimeIntent(ctx context.Context, req RunInput, history adapter.HistoryBuildResult, configs []models.ReplyIntentConfig) (callbacks.IntentTraceData, error) {
	...
	coverageRepair := runtimeQuestionRepairRequestFromContext(ctx)
	firstAttempt := 1
	if coverageRepair != nil {
		messages = append(messages, schema.SystemMessage(runtimeQuestionRepairInstruction(coverageRepair)))
		// Attempts 1-3 belong to initial detection and its existing retries.
		firstAttempt = 4
	}
	firstStartedAt := time.Now()
	firstReceiptOffset := len(usageCapture.Receipts())
	result, err := chatModel.Generate(intentCtx, messages)
	modelAttempt := firstAttempt
	if err != nil {
		recordIntentModelUsage(req, intentConfig, credentialRevision, nil, gatewayReceiptSince(usageCapture, firstReceiptOffset), firstAttempt, time.Since(firstStartedAt).Milliseconds(), err)
		if coverageRepair != nil {
			return callbacks.IntentTraceData{}, err
		}
		...
		recordIntentModelUsage(req, intentConfig, credentialRevision, result, gatewayReceiptSince(usageCapture, firstReceiptOffset), firstAttempt, time.Since(firstStartedAt).Milliseconds(), nil)
	}
...
func runtimeIntentDetectSystemPromptForProfile(profile *models.ReplyIntentProfile) string {
	...
	return strings.TrimSpace(runtimeQuestionFirstIntentInstruction() + "\n\n" + prompt + "\n\n" + schemaText + "\n\n本轮内部兼容扩展：intentTasks 允许额外输出 evidenceQuery 字符串，仅用于知识召回。clear + independent 的完整新问题直接使用当前 Task 的 text 原话检索，evidenceQuery 留空，resolvedText 保留原话，不摘短或混入其他 Task；resolved_from_context 的问题先在 resolvedText 中补全已知指代，以补全问题为检索依据，不得猜测指代或答案。上下文问题的 evidenceQuery 只能摘取检索依据中的连续原文片段，保留当前问题的对象、属性及仍有效的限定，不重新改写句子，不需要精简时留空。只有与所问公共服务无关的背景名称可以不摘取；房型设施和区域配置的对象、条件不能省略。不能新增执行主体、配送终点、时间、数量或使用范围，不用同义扩写替代客户表达。resolvedText 仍保留全部对象、区域、条件，交给 Judge 判断适用性。其他字段约定不变。\n上下文关系按下方实际提供的有界会话历史判断，不限于紧邻完整问答对；只有 answer_rejected 继续要求紧邻AI答复。同一个 URef 内也可以包含多个问题及其承接上下文，此时允许 independent + resolved_from_context，sourceRefs 只写该 URef 一次，不必虚构更早的消息或 previous 关系。问号、标点或表情也是客户原文，text 原样保留；与业务问题合并输入时可作为 context_only 的互动或真实上下文，不能代替业务 Task 的主要来源。")
}
...
func buildRuntimeIntentDetectUserPrompt(req RunInput, history adapter.HistoryBuildResult, configs []models.ReplyIntentConfig) string {
	...
	b.WriteString("\n\n判别纪律：只给“当前消息”分类；最近原始消息、媒体理解和长期记忆用于解释当前指代、承接和反馈，不把旧问题重新列为待答任务。短评、问号或表情要结合紧邻问答理解，不能只因为没有业务名词就当作新的招呼或泛化澄清。")
	b.WriteString("如果当前消息已经有独立的新主题，禁止沿用上一轮早餐、停车、投诉、安全、转人工等历史主题。")
	b.WriteString("普通常识、解释和日常建议即使与酒店无关，也可以是明确问题；使用 interaction/chat 和实际 objective，不虚构歧义。只有回答确实需要实时天气才归 weather_query，普通配色或偏好建议不因上一轮天气话题而强制调用工具。酒店业务仍使用原有知识分类。")
	b.WriteString(pmsQueryIntentInstruction())
	...
	b.WriteString("检索与系统提示使用同一来源规则：完整独立问题保留 text 原话，resolvedText 与 text 相同，evidenceQuery 留空；回指问题只补全各 Task 自己的对象和当前所问方面，不把同话题的其他问题合入查询。询问房型自身设施、房价或区域配置时，该对象就是检索目标，不能删掉。多题、单题使用相同规则，不另设背景摘除例外。")
	b.WriteString("先区分当前请求的执行主体与目的，再选类别；不能因话题包含外卖、订单或上一轮的动作就继续代操作。只有当前明确委托酒店替客户在第三方平台下单、购买、预订、叫车或联系外部商家时，输出 service_request/external_proxy_action，objective=action_request，needsKnowledge=false；这类请求直接说明平台能力边界，不为了寻找代操作办法检索无关知识。询问酒店自身设备或服务的存在性、能力、范围、规则属于 hotel_info，先检索知识；例如客户自己下单后询问酒店机器人配送范围，不是委托下单，resolvedText 不得添加原文未指定的酒店工作人员。酒店内部送物、补用品、维修、开门、换房、打扫等实际执行请求仍使用原有 service_request 子意图，禁止归入 external_proxy_action。")
...
func normalizeModelIntentTrace(intent callbacks.IntentTraceData, req RunInput, history adapter.HistoryBuildResult, configs []models.ReplyIntentConfig) callbacks.IntentTraceData {
	...
	case "hotel_info":
		intent.NeedsKnowledge = intentHasMixedHotelInfoTask(intent) ||
			(len(intent.IntentTasks) == 0 && !(intent.NeedsTool && isPMSRuntimeSubIntent(intent.SubIntent)))
	...
	for i := range intent.IntentTasks {
		...
		case "service_request":
			if isExternalProxyActionClassification(task.Intent, task.SubIntent, task.Objective) {
				task.NeedsKnowledge = false
				task.NeedsTool = false
			} else if isPMSRuntimeSubIntent(task.SubIntent) {
				task.NeedsTool = true
			} else {
				task.NeedsKnowledge = true
				task.NeedsTool = false
