...
func buildKnowledgeNoContextInstruction() string {
	return "知识库检索状态：当前没有从知识库检索到可用资料。\n" +
		...
		"3. 订单、房态、库存、会员、升房/换房可行性、差价、延迟退房和续住条件等只读 PMS 任务，继续依据 PMS 查询结果回答；PMS 查询成功后不能因为知识库没有资料而转人工。\n" +
		"4. 办入住、要小程序、要定位、问怎么去、补充房号、确认/取消等服务流程意图，应按当前系统已有工具/上下文继续处理或追问必要信息，不要走知识库兜底；只有客户明确要求人工、知识库明确要求转人工或存在严重安全风险时才进入人工路由。\n" +
		"5. 用户表达不清楚或缺少上下文时，像真人一样只追问一个最关键细节，不要一口气列很多字段；例如缺少手机号、订单号或日期时，只问一个能推进查询的字段。\n" +
		"6. 用户结合图片/语音/文件追问，且上下文已有图片/语音/文件等媒体理解结果，可以围绕确定内容继续问答；无法确定时说明需要哪一个具体信息，不要装作看懂。\n" +
		"7. 用户询问明确的酒店业务事实、规则、价格、流程、配置、时效、承诺、售后、退款、权限或政策，且当前没有资料支持时，不能编答案或告诉客户资料没写明；先说明暂时无法确认或追问一个关键字段，只有客户随后明确要求人工、知识库明确要求转人工或存在严重安全风险时才转接。\n" +
		"8. 不要用含糊确认、代记账、空泛跟进类话术替代真实处理；这些句式只会让客户感觉敷衍。\n" +
		"9. 不得编造，不得输出知识库或 PMS 未提供的具体事实、流程、承诺、价格、时效或政策；回复默认 1 句，像线上酒店接待微信聊天。"
}
...
func buildKnowledgeRetrievalErrorInstruction() string {
	return "知识库检索状态：知识库检索暂时不可用，当前没有可用的知识库资料。\n" +
		...
		"2. 寒暄、问候、感谢、确认、开玩笑、结束语、简单能力询问，可以自然短句回复。\n" +
		"3. PMS 只读查询仍可继续执行：订单、房态、库存、会员、升房/换房可行性、差价、延迟退房和续住条件优先依据 PMS 结果回答；查询成功后不能因为知识库暂时不可用而转人工。\n" +
		"4. 办入住、要小程序、要定位、问怎么去、补充房号、确认/取消等服务流程意图，应按当前系统已有工具/上下文继续处理或追问必要信息；只有客户明确要求人工、知识库明确要求转人工或存在严重安全风险时才进入人工路由。\n" +
		"5. 用户表达不清楚或缺少上下文时，像真人一样只追问一个最关键细节，不要一口气列很多字段；例如缺少手机号、订单号或日期时，只问一个能推进查询的字段。\n" +
		"6. 用户结合图片/语音/文件追问，且上下文已有图片/语音/文件等媒体理解结果，可以围绕确定内容继续问答；无法确定时说明需要哪一个具体信息，不要装作看懂。\n" +
		"7. 用户询问酒店业务事实、规则、价格、流程、配置、时效、承诺、售后、退款、权限或政策，且当前没有资料支持时，不能编答案或告诉客户资料没写明；先说明暂时无法确认或追问一个关键字段，只有客户随后明确要求人工、知识库明确要求转人工或存在严重安全风险时才转接。\n" +
		"8. 不要用含糊确认、代记账、空泛跟进类话术替代真实处理；这些句式只会让客户感觉敷衍。\n" +
		"9. 不得编造，不得输出知识库或 PMS 未提供的具体事实、流程、承诺、价格、时效或政策；回复默认 1 句，像线上酒店接待微信聊天。"
}
