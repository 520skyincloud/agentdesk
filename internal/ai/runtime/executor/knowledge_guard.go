...
func resolveKnowledgeFallbackReply(aiAgent models.AIAgent) string {
	...
	case enums.AIAgentFallbackModeSuggestRetry:
		return "这个我得再确认下。你补充下具体情况，我继续看。"
	default:
		return "这个我先记下，得让同事确认。"
	}
}
...
func buildKnowledgeRuntimeInstruction(answerMode enums.KnowledgeAnswerMode, fallbackReply string) string {
	...
	if fallbackReply == "" {
		fallbackReply = "这个我先记下，得让同事确认。"
	}
	if answerMode == enums.KnowledgeAnswerModeAssist {
		return "知识库回答约束：优先依据后续提供的知识片段回答，可以做轻度归纳，但不要编造片段中未提供的事实。具体事实、步骤、承诺、价格、时效、政策必须能被知识片段直接支持；如果片段不足，不要机械复读固定兜底，先按上下文追问一个关键细节；确实无法确认时才短句回复：" + fallbackReply
	}
	return "知识库回答约束：本轮只能依据后续提供的知识片段回答，不得使用模型常识补充未提供的事实，不得输出知识片段外的具体事实、步骤、承诺、价格、时效或政策。片段不足时不要套模板，优先追问一个能推进问题的关键细节；确实无法确认时才短句回复：" + fallbackReply
}
...
func buildKnowledgeNoContextInstruction(fallbackReply string) string {
	...
	return "知识库检索状态：当前没有从知识库检索到可用资料。\n" +
		...
		"5. 用户结合图片/语音/文件追问，且上下文已有图片/语音/文件等媒体理解结果，可以围绕确定内容继续问答；无法确定时说明需要确认，不要装作看懂。\n" +
		"6. 只有当用户询问酒店业务事实、规则、价格、流程、配置、时效、承诺、售后、退款、权限或政策，且当前没有资料支持时，才允许短句确认/追问/转人工；必须避免每次都复读同一句。可参考但不要死抄：" + fallbackReply + "\n" +
		"7. 不得编造，不得输出知识库未提供的具体事实、流程、承诺、价格、时效或政策；回复默认 1 句，像前台同事微信聊天。"
}
