...
func buildCurrentTurnBoundaryInstruction(req RunInput, history adapter.HistoryBuildResult, intent callbacks.IntentTraceData) string {
	...
	parts := []string{
		...
		"动作安全：没有工具、资源提交、接待路由或明确系统执行结果时，不能承诺或暗示任何真实动作、内部核实、通知转告、登记安排、现场查看、后续跟进或已完成状态。知识库没写明时，只能说明当前资料或能力边界，不索要房号、订单、姓名或手机号；真要人工时必须进入人工意图/接待路由，不能口头假装后续有人处理。",
	}
	currentLine := "当前客户消息"
	...
	if intent.PrimaryIntent == "hotel_info" || intent.PrimaryIntent == "service_request" {
		parts = append(parts, "酒店信息/服务请求：只围绕当前问题使用知识库结果，不要把同一会话里的其他酒店问题一起回答。知识库已经给出答案时必须直接回答，不能说正在查、稍后查、内部确认或后续处理。如果当前问题里某一项知识库没有明确写明，只能说“当前资料没写明”，不能索要房号、订单、姓名或手机号。")
	}
	...
	if intent.PrimaryIntent == "service_request" {
		parts = append(parts, "服务请求回复结构：先看知识库有没有自助路径或处理边界；没有明确答案时只说明当前资料或能力边界，不索要房号、订单、姓名或手机号，不得自行转人工。没有路由或工具结果时，禁止表达动作已执行、内部确认或后续有人处理。房型升级或换房无法执行时，要直接说明当前不能改房型、不能确认实时房态，先不用客户提供订单信息，不引导联系不存在的前台。")
	}
	if looksLikeReturningCustomerTurn(currentText) {
		parts = append(parts, "当前消息像是跨天/隔一段时间后重新咨询；旧消息或长期记忆里的房号、入住事实都已过期，回复中不能沿用旧房号。当前问题能靠知识回答就直接回答，不能为了确认房号而追问；只有真实工具或接待路由明确要求 roomNo 时，才可询问当前房号。回复必须点名当前问题本身，例如当前问电视就要提到电视，不能只欢迎回来或只问是否到房间。")
	}
