...
func buildCurrentTurnBoundaryInstructionForReplyPlan(req RunInput, history adapter.HistoryBuildResult, intent callbacks.IntentTraceData, replyPlan callbacks.ReplyPlanTraceData) string {
	...
	parts := []string{
		...
		"动作安全：没有工具、资源提交、接待路由或明确系统执行结果时，不能承诺或暗示任何真实动作、内部核实、通知转告、登记安排、现场查看、后续跟进或已完成状态。明确的酒店业务问题如果知识库没有可用答案，仍保留当前任务；能由 PMS 只读结果回答就按 PMS 回答，其他情况只说明暂时无法确认或追问一个最关键字段，不能因为未命中自动转人工。",
	}
	currentLine := "当前客户消息"
	...
	if intent.PrimaryIntent == "hotel_info" || intent.PrimaryIntent == "service_request" {
		parts = append(parts, "酒店信息/服务请求：只围绕当前问题使用知识库和 PMS 结果，不要把同一会话里的其他酒店问题一起回答。知识库或 PMS 已经给出答案时必须直接回答，不能说正在查、稍后查、内部确认或后续处理。如果明确的酒店业务问题没有可用答案，保留该问题并自然说明暂时无法确认或追问一个关键字段；只有客户明确要求人工、知识库明确要求转人工或存在严重安全风险时才进入接待路由。")
	}
	if len(activeTaskTexts) > 1 || (len(replyPlan.TaskPlans) == 0 && len(intent.IntentTasks) > 1) {
		parts = append(parts, "当前轮包含连续多问：必须按客户消息顺序逐项覆盖当前轮每个问题；不要只回答主意图或最后一个问题。已检索到的知识或 PMS 事实必须直接答；没有可用答案的酒店业务项也要保留在回复中，只说明暂时无法确认或追问一个关键字段，不能说“资料没写明”“帮你查/我查一下”，也不能因此自动转人工。")
	}
	if intent.PrimaryIntent == "service_request" {
		parts = append(parts, "服务请求回复结构：先看知识库有没有自助路径或处理边界；涉及订单、房态、库存、会员或差价时同时使用只读 PMS。没有明确答案时追问一个必要字段或说明暂时无法确认；只有客户明确要求人工、知识库明确要求转人工或存在严重安全风险时进入接待路由。没有路由或工具结果时，禁止表达动作已执行、内部确认或后续有人处理。")
	}
