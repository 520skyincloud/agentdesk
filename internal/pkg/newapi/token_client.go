...
const tokenQueryResponseLimit = 4 << 20
...
func NewTokenClient(baseURL, apiKey string, timeout time.Duration) (*TokenClient, error) {
...
func (c *TokenClient) ListUsageLogs(ctx context.Context, startTimestamp, endTimestamp int64) ([]TokenUsageLog, error) {
	values := url.Values{}
	if startTimestamp > 0 {
...
func (c *TokenClient) get(ctx context.Context, path string) ([]byte, error) {
	...
	body, err := io.ReadAll(io.LimitReader(resp.Body, tokenQueryResponseLimit))
	if err != nil {
