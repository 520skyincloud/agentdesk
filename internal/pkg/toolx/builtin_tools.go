...
`),
	...
	BuiltinPMSQuery = ToolSpec{
		...
		Name:          "pms_query",
		Title:         "查询 PMS",
		Description:   "只读查询 PMS 中的订单、实时房态、库存、续住候选、会员信息及等级权益，不执行续住或其他写操作。",
		SourceType:    enums.ToolSourceTypeBuiltin,
		DirectAccess:  true,
		RuntimeStatic: true,
		Appendix: strings.TrimSpace(`
当用户询问订单、入住状态、实时房态、可售库存、会员等级、会员状态或会员权益时，优先调用 pms_query。
当前有效订单可按手机号或会员编号/协议公司编号查询；两个条件同时提供时按精确组合匹配。只查会员基础信息使用 member_info_by_phone；查询当前客户权益、升级或保级规则使用 member_benefits_by_phone，工具会从准确手机号自动查会员，再使用实际等级查询规则。跨轮追问也使用已有客户手机号重新查询，缺少手机号才追问；不要求客户提供等级 ID，不猜测等级编码。
会员冻结/挂失、等级不存在或 gradeAvailable=false 时如实说明状态，不能承诺权益可用。会员等级有效期为空不代表失效；依据返回的有效期事实回答。权益和等级规则仅代表配置，不是客户实际订单报价，也不代表已升房、延退、发券或已入账。
升级和保级规则只解释实际返回等级的配置，不推测下一等级，不因缺少消费或入住历史就认定客户已达标。
工具只允许只读查询，不提交续住、改房、排房、换房、延退、改价或占房，不得生成或宣称已办理的结果。不要把可售库存说成已经排房，不要把空价格说成免费。没有实名信息查询接口，不能宣称已完成身份核验。
工具返回 unavailable 或 unsupported 时不得宣称查询或办理成功；用简短客户语言说明当前限制，不转发内部字段、接口地址或认证错误。
工具返回 partial 时保留已查询到的会员事实，只说明缺失部分尚未查到，不把部分成功说成全部失败或全部成功。
`),
	}
...
6. 对送水、拖鞋、毛巾、加被、打扫、维修、行李、叫醒等需要真实员工动作的 SERVICE_TASK，禁止口头承诺已经安排；必须先补齐房号、具体事项、数量/位置、期望时间，随后按门店知识继续回答，或在客户明确要求人工时再建单/转人工。只读的订单、房态、库存、升房/换房可行性、差价和延退咨询不属于 SERVICE_TASK。
7. 退款、赔偿、普通投诉、订单异常、隐私授权、价格争议和安全问题先结合知识库与 PMS 只读事实给出可确认的方案或缺失字段；只有客户明确要求人工或知识库明确要求转人工时，才调用人工路由。
`),
	}
...
1. 只有在用户明确要求人工客服，或知识库明确要求转人工时，才调用该工具；普通服务请求、投诉、退款、赔偿、订单异常、价格争议、安全问题和只读 PMS 咨询不得仅因需要后续执行而直接转人工。
2. 调用前先尽量整理清楚转人工原因；如果理由含糊，先追问或澄清是否需要人工，不要直接转人工。
