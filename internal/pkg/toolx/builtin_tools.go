...
当前有效订单可按手机号或会员编号/协议公司编号查询；两个条件同时提供时按精确组合匹配。查询当前客户会员权益时，先用准确 phone 调用 member_info_by_phone，再用响应中的真实 gradeId 调用 member_benefits_by_grade；gradeCode 只是 gradeId 的兼容别名，不得用金卡等中文等级名称猜测编码。
