...
func tenantScopedUniqueIndexSpecs() []tenantScopedUniqueIndexSpec {
	return []tenantScopedUniqueIndexSpec{
		{
			model: &models.Company{}, currentName: "uk_company_tenant_name",
			currentFields: []string{"tenant_id", "name"}, legacyName: "uk_company_name", legacyFields: []string{"name"},
		},
		{
			model: &models.Store{}, currentName: "uk_store_tenant_code",
			currentFields: []string{"tenant_id", "store_code"}, legacyName: "idx_t_store_store_code", legacyFields: []string{"store_code"},
		},
		{
			model: &models.AgentProfile{}, currentName: "uk_agent_profile_tenant_code",
			currentFields: []string{"tenant_id", "agent_code"}, legacyName: "idx_t_agent_profile_agent_code", legacyFields: []string{"agent_code"},
		},
	}
}

// retireLegacyGlobalUniqueIndexes runs after AutoMigrate has created the
// replacement indexes. GORM intentionally does not remove obsolete indexes.
func retireLegacyGlobalUniqueIndexes(db *gorm.DB) error {
	for _, spec := range tenantScopedUniqueIndexSpecs() {
		...
		if err := requireUniqueIndex(db, spec.model, spec.legacyName, spec.legacyFields); err != nil {
			return fmt.Errorf("refuse to retire legacy index %s: %w", spec.legacyName, err)
		}
		if err := db.Migrator().DropIndex(spec.model, spec.legacyName); err != nil {
			return fmt.Errorf("drop legacy unique index %s failed: %w", spec.legacyName, err)
		}
	}
	return nil
}
