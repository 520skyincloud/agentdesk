...
func (r *fastGPTStoreTenantRepository) MarkActivationFailedIfAppliedRevision(db *gorm.DB, tenantID, storeID, bindingID, credentialRevision int64, columns map[string]any) error {
	if db == nil || tenantID <= 0 || storeID <= 0 || bindingID <= 0 || credentialRevision <= 0 {
		return errors.New("fastgpt activation failure scope is required")
	}
	return db.Model(&models.FastGPTStoreTenant{}).
		Where("tenant_id = ? AND store_id = ? AND applied_store_staff_binding_id = ? AND applied_credential_revision = ?", tenantID, storeID, bindingID, credentialRevision).
		Updates(columns).Error
}

func (r *fastGPTStoreTenantRepository) ApplyTargetRevisions(db *gorm.DB, tenantID, storeID, bindingID, profileID, profileRevision, credentialRevision int64, columns map[string]any) (bool, error) {
	if db == nil || tenantID <= 0 || storeID <= 0 || bindingID <= 0 || profileID <= 0 || profileRevision <= 0 || credentialRevision <= 0 {
		return false, errors.New("FastGPT target revision scope is required")
	}
	result := db.Model(&models.FastGPTStoreTenant{}).
		Where("tenant_id = ? AND store_id = ? AND target_store_staff_binding_id = ? AND target_profile_id = ? AND target_profile_revision = ? AND target_credential_revision = ?",
			tenantID, storeID, bindingID, profileID, profileRevision, credentialRevision).
		Updates(columns)
...
func (r *fastGPTStoreTenantRepository) Save(db *gorm.DB, item *models.FastGPTStoreTenant) error {
	return db.Clauses(clause.OnConflict{
		...
		DoUpdates: clause.AssignmentColumns([]string{
			...
			"target_store_staff_binding_id", "applied_store_staff_binding_id",
